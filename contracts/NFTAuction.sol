...
contract NFTAuction {
    ...
    function createAuction(
        ...
        external {
            ...
            require(_duration > 86400 && _duration < 86400, '_duration must be between 1 to 10 days');
            auction[nextId] = Auction(
                nextId,
                msg.sender,
                _name,
                _description,
    ...
    function createOffer(uint _auctionId) external payable auctionExists(_auctionId)  {
        ...
        Offer storage bestOffer = offer[auction.bestOfferId];
        require(now < auction.end, 'Auction has expired');
        require(msg.value >= auction.min && msg.value > bestOffer.price, 'msg.value must be superior to _min and bestOffer');
    ...
    modifier auctionExist(uint _auctionId) {
        require(auctionId > 0 && auctionId < nextAuctionId, 'Auction does not exist');
        _;
    }
}